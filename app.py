...
if "hasil_topsis" not in st.session_state:
    st.session_state.hasil_topsis = None
    ...
    with st.spinner("🔎 Mendeteksi lokasi..."):
        ...
        if geo_res["status"] == "OK":
            ...
            if not df_all.empty:
                ...
                st.session_state.hasil_topsis = df_tampil

        else:
            st.error(f"❌ Lokasi tidak ditemukan: {geo_res['status']}")

# === Tampilkan dan Filter Hasil ===
if st.session_state.hasil_topsis is not None:
    ...
    df_tampil = st.session_state.hasil_topsis
